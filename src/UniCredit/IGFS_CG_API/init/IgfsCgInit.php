...
class IgfsCgInit extends BaseIgfsCgInit {
	...
	protected function buildRequest() {
		...
		if ($this->payInstrAddData != NULL) {
			$sb = "";
			foreach ($this->payInstrAddData as $item) {
				$sb .= $item->toXml("payInstrAddData");
			}
			$request = $this->replaceRequest($request, "{payInstrAddData}", $sb);
		} else
			$request = $this->replaceRequest($request, "{payInstrAddData}", "");
